...
QList<qint32> findValue(const QList<qint32> & lst_container,qint32 nval)
{
    QList<qint32> ret;
    qint32 nIndex = 0;
    QListIterator<qint32> iEveryItem(lst_container);
    while(iEveryItem.hasNext()){
        qint32 nItem = iEveryItem.next();
        if(nItem == nval){
           ret.append(nIndex);
        }
        nIndex++;
...
qint32 findMax(const QList<qint32> & lst_container, const QList<qint32> & lst_indexs){

    qint32 ret = 0;

    QListIterator<qint32> iEveryItem(lst_container);
    while(iEveryItem.hasNext()){
       qint32 nIndex =  iEveryItem.next();
       if(ret < lst_container[nIndex]){
           ret = lst_container[nIndex];
       }
    }
    return ret;
}
...
void TrafficAlg::process()
{
    ...
    const QList<qint32> x_data = LMS_data_1.xdata();
    const QList<qint32> z_data = LMS_data_1.zdata();


    QList<QPointF> frame_data;
    for(qint32 nIndex = m_min_x;nIndex < m_max_x; nIndex+=10){

        qint32 x_tmp =nIndex+m_min_x;
        qint32 z_tmp = 0;

        QList<qint32> lst_indexs = findValue(x_data,nIndex);
        if(lst_indexs.size()){
          z_tmp = findMax(z_data,lst_indexs);
        }

        frame_data<<QPointF(x_tmp/1000.0,z_tmp/1000.0);
    }

    emit signal_Frame(frame_data);
}
#else
void TrafficAlg::process()
{
