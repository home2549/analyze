...
const int sampleCountX = 90;
const int sampleCountZ = 90;
    ...
    : m_graph(surface)
{
    ...
    m_timer_sky = new QTimer(this);
    connect(m_timer_sky,SIGNAL(timeout()),SLOT(changeY()));
    m_timer_sky->start(100);
...
void SurfaceGraph::slotShow(const QList<QList<QVector3D> > & ndata)
{
    QSurfaceDataArray *dataArray = new QSurfaceDataArray;
    dataArray->reserve(ndata.size());
  //  qint32 tCount = ndata.size();

    QListIterator<QList<QVector3D> > iEveryItem(ndata);
    while(iEveryItem.hasNext()){
      const QList<QVector3D> & ref_nitem =  iEveryItem.next();
       QSurfaceDataRow *newRow = new QSurfaceDataRow(ref_nitem.size());

       QListIterator<QVector3D> iEverySubItem(ref_nitem);
       while(iEverySubItem.hasNext()){
          const QVector3D & nSubItem = iEverySubItem.next();
          *newRow<<QSurfaceDataItem(nSubItem);
       }
...
#include <QDebug>
void SurfaceGraph::changeY()
{
//    rand_sky = (qrand()%5)/10.0;
//    qDebug()<<rand_sky;
//    fillSqrtSinProxy();

    QList<QList<QVector3D> > tst;
    for(qint32 nRnum = 0;nRnum < 50;nRnum++){

        qreal z_val = nRnum*0.32;

        QList<QVector3D> c_val;
        for(qint32 nCnum = 0;nCnum < 90;nCnum++){
            QVector3D pt;
            pt.setZ(z_val);
            pt.setX(nCnum*0.32);
            pt.setY(qSin((nCnum+angle_base)*0.32) + 1.01);
            c_val<<pt;
        }
        //slotAppendRow(c_val);
        tst<<c_val;
...
void SurfaceGraph::slotAppendRow(const QList<QVector2D> & ndata)
{
   const QSurfaceDataArray *dataArray =  m_sqrtSinProxy->array();
//   if(dataArray->size() > 50){

//       const QSurfaceDataRow * nItem = dataArray->first();
//       (*dataArray).pop_front();
//       delete nItem;
//   }
   qreal step_z = 0.05;
   qint32 z_index = dataArray->size();
   qreal z_val = z_index*step_z;
   QSurfaceDataRow *nSurfaceItem = new QSurfaceDataRow();
   QListIterator<QVector2D> iEveryItem(ndata);
   while(iEveryItem.hasNext()){
        const QVector2D & nItem = iEveryItem.next();
        *nSurfaceItem<<QSurfaceDataItem(QVector3D(nItem.x(),nItem.y(),z_val));
   }
   m_sqrtSinProxy->insertRow(z_index,nSurfaceItem);
}
